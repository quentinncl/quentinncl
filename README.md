[![linkedin logo](https://img.shields.io/static/v1?message=Find+on+me+LinkedIn&logo=linkedin&label=&color=0077B5&logoColor=white&labelColor=&style=for-the-badge)](https://www.linkedin.com/in/quentinncl/)

<h1>Hey, moi c'est Quentin 👋</h1>

### 🧑‍💻 À propos de moi

Développeur full-stack passionné, entrepreneur et formateur basé en **Nouvelle-Aquitaine, France**.

- 🏢 Fondateur de **[Sinap's Conseils](https://github.com/sinaps-conseils)** — éditeur du SaaS **Rezall**, plateforme B2B de gestion d'infrastructure FTTH en France
- 💼 Consultant / lead dev via **Digital Associates**, en mission chez des clients grands comptes
- 🎓 Formateur en développement web : **React**, **GraphQL**, **Clean Code**, **Cloud**
- 🚀 Toujours en train de construire quelque chose — SaaS, outils internes, homelab...
- 📍 Niort, France

---

### 🛠 Stack principale

#### Frontend
<div align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/react/react-original.svg" height="40" alt="react logo"/>
<img width="12" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/typescript/typescript-original.svg" height="40" alt="typescript logo"/>
<img width="12" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nextjs/nextjs-original.svg" height="40" alt="nextjs logo"/>
<img width="12" />
</div>

#### Backend & API
<div align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nodejs/nodejs-original.svg" height="40" alt="nodejs logo"/>
<img width="12" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nestjs/nestjs-original.svg" height="40" alt="nestjs logo"/>
<img width="12" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/graphql/graphql-plain.svg" height="40" alt="graphql logo"/>
<img width="12" />
</div>

#### Data & Infra
<div align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mongodb/mongodb-original.svg" height="40" alt="mongo logo"/>
<img width="12" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/postgresql/postgresql-original.svg" height="40" alt="postgresql logo"/>
<img width="12" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/docker/docker-original.svg" height="40" alt="docker logo"/>
<img width="12" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/kubernetes/kubernetes-original.svg" height="40" alt="kubernetes logo"/>
<img width="12" />
</div>

---

### 🎓 Ce que j'enseigne

| Sujet | Niveau |
|---|---|
| React & TypeScript | Intermédiaire → Avancé |
| React Native / Expo | Intermédiaire → Avancé |
| GraphQL (schéma, resolvers, clients) | Débutant → Avancé |
| Clean Code & bonnes pratiques | Tous niveaux |
| Cloud & architectures modernes | Intermédiaire |

Et plein d'autres trucs cools ! Plus d'infos [ici](https://othila-academy.com) ❤️

---

### 🔥 Stats

[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs?username=quentinncl&locale=en&hide_title=false&layout=compact&card_width=320&langs_count=5&theme=dracula&hide_border=true&order=2)](https://github.com/quentinncl)
[![Streak](https://streak-stats.demolab.com?user=quentinncl&locale=en&mode=daily&theme=dark&hide_border=false&border_radius=5&order=3)](https://github.com/quentinncl)
